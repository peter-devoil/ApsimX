...
namespace Models.PMF.Arbitrator
{
    ...
    public class C4WaterUptakeMethod : Model, IUptakeMethod, IStructureDependency
    {
        ...
        public List<ZoneWaterAndN> GetUptakeEstimates(SoilState soilstate, IArbitration[] Organs)
        {
            ...
            // The overall fraction of water to take
            ...
            for (int i = 0; i < supplies.Count; i++)
            {
                ...
                for (int j = 0; j < supplies[i].Length; j++)
                {
                    ...
                    if (maxAmt > uptake.Water[j]) {
                        var pref = Math.Max(0, (maxAmt - uptake.Water[j]) *  (ExtractionPreference is null ? 0 : ExtractionPreference.Value())); // the difference between uniform extr, and maximum preferred
        ...
        /// <summary>
        /// Preference for water extraction - top down (1) to uniform (0)
        /// </summary>
        /// <remarks>
        /// The 
        /// </remarks>
